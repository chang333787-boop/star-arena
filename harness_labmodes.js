...
script+=`;globalThis.__api={ STATE, keysDown,
  ...
  SV_UPS, svStart, svSelect, svEnd, svUpdate, svRender, svKey, svPick, svApplyUp, svRollChoices, svKill, svHurt,
...
run("생존자: v1.95 7분판 — 140초 보스·죽음의 뭉게대왕 피날레·처치 랭킹", ()=>{
  ...
  const b=JSON.parse(LS["starArena.lab.svBest.v2"]);
  check("420초(7분) → 완주 승리+기록", SV.phase==="win" && b.wins>=1);
  check("처치 기록 bestK + 랭킹 svk2", b.bestK>=137 && (api.labRank.svk2||[]).length>=1);
  // 사신에게 잡히면 = 완주(win)
...
run("생존자: v1.94 어려움 — 회복 카드 제외 + 방어막 1회용/30초 만료", ()=>{
  // 어려움: 여러 번 롤해도 regen/maxhp 안 나옴
  api.svStart(true); const SV=api.SV;
  let sawRecover=false;
  for(let i=0;i<80;i++){ const ch=api.svRollChoices(); for(const c of ch){ if(c.k==="regen"||c.k==="maxhp") sawRecover=true; } }
  check("어려움: 회복 카드(regen/maxhp) 80회 롤에도 안 나옴", sawRecover===false);
  check("어려움: 방어막 카드는 롤 풀에 있음", api.SV_UPS.some(u=>u.k==="shield"));
  // 방어막 픽 → 즉시 1회 방어막 + 30초 만료
  SV.p.shield=false; SV.p.shieldExpire=undefined;
  api.svApplyUp("shield");
  check("어려움: 픽 즉시 방어막 ON + 30초 만료", SV.p.shield===true && Math.abs(SV.p.shieldExpire-30)<0.01);
  // 30초 방치 → 사라짐
  for(let i=0;i<31*60 && SV.p.shield;i++) api.svUpdate(1/60);
  check("30초 안 쓰면 방어막 사라짐", SV.p.shield===false);
  // 재충전 없음: 방어막 픽 → 한 방 막음 → 시간 지나도 안 돌아옴
  api.svStart(true); const S2=api.SV;
  api.svApplyUp("shield"); S2.p.ifr=0;
  api.svHurt(3);
  check("어려움: 방어막이 한 방 막음(안 죽음)", S2.p.shield===false && S2.phase==="play");
  S2.foes.length=0; S2.spawnT=9999;
  for(let i=0;i<15*60;i++) api.svUpdate(1/60);
  check("어려움: 재충전 없음(15초 지나도 방어막 안 돌아옴)", S2.p.shield===false);
  // 보통 모드: 방어막 재충전됨(회귀)
  api.svStart(false); const S3=api.SV;
  api.svApplyUp("shield"); S3.foes.length=0; S3.spawnT=9999;
  for(let i=0;i<14*60 && !S3.p.shield;i++) api.svUpdate(1/60);
  check("보통: 방어막 재충전됨", S3.p.shield===true);
  api.svKey("Escape");
});
run("생존자: 난이도 선택 화면", ()=>{
  api.svSelect();
