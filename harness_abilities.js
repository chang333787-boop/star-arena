...
function setup(charId, foeDx){
  ...
  e.char=null; e.characterId=null;   // OVERNIGHT-1: 봇 캐릭터 패시브(별골렘 10%↓ 등) 중립화 — 정밀 피해 검증용
...
run("눈꽃 3발/탄약1(중앙 직선)", ()=>{                        // OVERNIGHT-1 2-2: 발수 3(홀수) — 중앙 1발이 정확히 직선
  const {p,e}=setup("student_04", 90);
  api.setSuper(0);
  const ammo0=p.ammo;
  api.keysDown.add("KeyZ"); frames(1); api.keysDown.clear();
  check("방울 3발 생성", api.bullets.length===3);
  check("중앙 탄은 직선(vy=0)", api.bullets.some(b=>Math.abs(b.vy)<1e-9));
...
console.log("=== 6) 별골렘 X 방패 생성(OVERNIGHT-1: 흡수막 110·4초)·막은 피해로 궁 충전·상시 10%↓ ===");
run("흡수막", ()=>{
  const {p}=setup("student_06", 400);
  api.castSpecialFor(p, api.envLocal());
  check("흡수막 110·4초·쿨8s", p.shieldAbsorb===110 && p.shieldAbsorbTimer>3.9 && p.specialCd<=8);
  const g0=api.superGauge; const hp0=p.hp;
  api.applyDamage(p, 10, "enemy", 999, p.x, p.y);
  check("10 피해 전부 흡수(HP 불변·막 100 잔여)", p.hp===hp0 && p.shieldAbsorb===100);
  check("수호자 패시브: 막은 10×0.6=6 게이지", Math.abs(api.superGauge-(g0+6))<1e-6);
  api.applyDamage(p, 200, "enemy", 999, p.x, p.y);
  check("막 100 흡수 후 초과 100×0.9=90만 HP 피해(상시 10%↓)", Math.abs((hp0-p.hp)-90)<1e-6 && p.shieldAbsorb===0);
...
run("흡수막 아군 부여(조준 방향)", ()=>{
  ...
  p.specialCd=0;
  api.castSpecialFor(p, api.envLocal());
  check("본인+조준 아군 모두 흡수막", p.shieldAbsorb===110 && ally.shieldAbsorb===110);
});
run("별골렘 상시 피해 10%↓(방패 없이)", ()=>{
  const {p}=setup("student_06", 400);
  const hp0=p.hp;
  api.applyDamage(p, 20, "enemy", 999, p.x, p.y);
  check("20 피해 → 18", Math.abs((hp0-p.hp)-18)<1e-6);
});
...
run("달이 C(OVERNIGHT-1 2-5): 0.35초 시전 후 일직선(900×56) 적 55 + 저격선", ()=>{
  const {p,e}=setup("student_02", 300);
  p.facing=0; e.y=p.y;                       // 일직선 위
  api.setSuper(100);
  api.handleKeyPress("KeyC");
  check("시전 예고(snipeCast) 생성 — 즉발 아님", api.skillZones.some(z=>z.type==="snipeCast") && e.hp===e.maxHp);
  check("게이지는 시전 시점에 소진", api.superGauge===0);
  api.tickSkillZones(0.36, api.envLocal());  // wind-up 경과 → 발사
  check("시전 완료 후 라인 위 적 55 피해", Math.abs((e.maxHp-e.hp)-55)<1e-6);
  check("저격선 이펙트 존(beam)", api.skillZones.some(z=>z.type==="beam"));
  // 시전 중 사망하면 불발
  const {p:p2,e:e2}=setup("student_02", 300);
  p2.facing=0; e2.y=p2.y; api.setSuper(100);
  api.handleKeyPress("KeyC");
  p2.dead=true;
  api.tickSkillZones(0.36, api.envLocal());
  check("시전자 사망 시 불발", e2.hp===e2.maxHp);
  p2.dead=false;
});
