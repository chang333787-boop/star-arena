...
function makeMockDB(){
  ...
  function ref(path){ path=path||""; return {
    ...
    set(v){ const errs=[]; fbValidate(v,"set("+path+")",errs); if(errs.length) throw new Error("FB_REJECT "+errs[0]); setAt(path, resolveTS(clone(v))); fire(); return thenable(); },
    update(obj){ const errs=[]; fbValidate(obj,"update("+path+")",errs); if(errs.length) throw new Error("FB_REJECT "+errs[0]); for(const k in obj) setAt(path+"/"+k, resolveTS(clone(obj[k]))); fire(); return thenable(); },
...
run("로비 '온라인 센터'(row8) Enter → online_menu(소프트락 아님)", ()=>{
  api.setState(api.STATE.START); api.setMenuIndex(8); api.handleKeyPress("Enter");  // v1.26 3층 IA 로비: row7=온라인 센터('친구와' 그룹)
