...
- **v1.15 슬라이싱/렌더 정리**: 배경 제거를 밝기/회색 경계 플러드+최대 연결요소 보존으로 교체 → 발밑 흰색/그림자 덩어리·흩어진 효과 찌꺼기 제거. 캐릭터별 5프레임을 같은 캔버스·발 기준으로 정렬(union box)하고, 렌더를 발 기준 앵커로 변경 → attack 안 잘림·프레임 크기/위치 일정.
- 남은 제한: 발사체/적중효과 미적용(색 원형 fallback), 온라인 hit 프레임 미표시, 던지는 무기(부메랑 등)는 attack에서 손에서 떨어진 발사체는 생략(게임이 자체 탄환 그림). 다시 자르려면 `python tools/slice_assets.py` (상단 MOTION/MASTER 좌표 조정 가능).
