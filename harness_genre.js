...
run("감염: 접촉 → 전환 + 2초 재감염 쿨(AC)", ()=>{
  const all=api.allFighters();
  for(const f of all) f.tagShield=0;   // BIG-BATCH-2 P1: 별골렘 방어막은 별도 테스트 — 여기선 순수 감염 판정
...
run("스킬: 럭키 X=모드 대시 · C 잠금 유지 (BIG-BATCH-2 P1)", ()=>{
  api.player.specialCd=0;
  const base=api.effSpeed(api.player);
  api.useCharacterSpecial();
  check("럭키 X → 반짝 대시(쿨 시작)", api.player.tagDashT>0 && api.player.specialCd>0);
  check("대시 중 실효 이속 증가", api.effSpeed(api.player) > base*1.2);
  api.setSuper(100); api.useCharacterSuper();
  check("C 발동 0(잠금) — 게이지 유지", api.superGauge===100);
});
run("별골렘 방어막: 1회 잡기 무효 + 짧은 면역 (BIG-BATCH-2 P1)", ()=>{
  const all=api.allFighters();
  const tg=all.find(f=>f.isTagger), r=all.find(f=>!f.isTagger && !f.isPlayer);
  if(!tg||!r){ check("술래/도망자 존재", false); return; }
  r.tagShield=1; r.tagImmuneT=0; tg.tagCd=0;
  r.x=tg.x; r.y=tg.y; api.tickRule(0.016);
  check("방어막이 잡기를 막음(감염 안 됨)", r.isTagger!==true && r.tagShield===0);
  check("직후 짧은 면역", r.tagImmuneT>0);
  r.tagImmuneT=0;   // 면역 소진(감쇠는 tickStatus=프레임 루프 소관 — 여기선 직접 소거)
  tg.tagCd=0; r.x=tg.x; r.y=tg.y; api.tickRule(0.016);
  check("면역 소진 후엔 감염", r.isTagger===true);
});
run("도망자 버프: 술래 수 비례 이속 (BIG-BATCH-2 P1)", ()=>{
  api.tickRule(0.016);
  const all=api.allFighters(), taggers=all.filter(f=>f.isTagger).length;
  check("runnerMul 계산("+taggers+"술래)", api.RULE.runnerMul>=1 && (taggers<2 || api.RULE.runnerMul>1));
});
